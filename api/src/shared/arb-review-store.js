...
function renderMarkdownExportBody(pack) {
  ...
  const blockers       = findings.filter((f) => ["Critical","High"].includes(f.severity) && f.status !== "Closed");
  ...
  const nextReview     = new Date(); nextReview.setDate(nextReview.getDate() + 90);
  ...
  const lines = [
    ...
    `> **Customer:** ${cust.name}  |  **Review ID:** ${meta.reviewId}  `,
    ...
    "## Table of Contents",
    "",
    "1. [Review Metadata](#review-metadata)",
    "2. [Governance Decision & Approval Conditions](#governance-decision--approval-conditions)",
    "3. [Assessment Scope & Methodology](#assessment-scope--methodology)",
    ...
    "12. [Uploaded Inputs](#uploaded-inputs)",
    "",
    "---",
    "",
    "## Review Metadata",
    ...
    `| **Overall Score** | ${es.overallScore != null ? `${es.overallScore} / 100 (${es.scoreBand || "—"})` : "TBD"} |`,
    ...
    `| **Critical Findings (open)** | ${sevCounts.Critical} |`,
    `| **High Findings (open)** | ${sevCounts.High} |`,
  ...
  // Governance Decision
  lines.push("", "---", "", "## Governance Decision & Approval Conditions", "");
  const decStatus = dc.reviewerDecision || "Pending Review";
  lines.push(`**DECISION: ${decStatus}**`, "");
  if (dc.governancePosture) lines.push(`- **Governance Posture:** ${dc.governancePosture}`);
  if (hasDecision) {
    lines.push(
  ...
  if (dc.rationale) lines.push(`- **Rationale:** ${dc.rationale}`);
  if (dc.governanceWarning) lines.push(`- ⚠ **Governance Warning:** ${dc.governanceWarning}`);

  if (blockers.length > 0) {
    lines.push("", `### Approval Conditions — ${blockers.length} blocking item${blockers.length !== 1 ? "s" : ""} must be resolved`, "");
    for (const f of blockers) {
      lines.push(`- **[${f.severity}]** ${f.title}${f.recommendation ? `  →  _${f.recommendation}_` : ""}`);
    }
  } else {
    lines.push("", "_No Critical or High severity blockers. Decision can proceed._");
  }
  ...
  lines.push(
    ...
    "| Parameter | Value |",
    "|-----------|-------|",
    "| **Frameworks Applied** | Azure Well-Architected Framework (WAF) · Azure Cloud Adoption Framework (CAF) |",
    `| **Domains Assessed** | ${domainCount} domain${domainCount !== 1 ? "s" : ""} |`,
    `| **Assessment Duration** | ${meta.reviewDuration || "Not recorded"} |`,
    "| **Review Approach** | AI-assisted document extraction, rule-based scoring, evidence cross-referencing |",
    `| **Next Review Recommended** | ${nextReviewStr} |`,
    "",
    "**Scoring Bands:**",
    "",
    ...
    "| Critical | 0–49% | Fundamental issues identified. May not meet governance standards. |",
  );

  // Scorecard (table format)
  lines.push("", "---", "", "## Scorecard", "",
    `**Overall: ${sc.percentage ?? 0}% — ${sc.totalScore ?? 0} / ${sc.maxScore ?? 100} points**`,
    "",
    "| Domain | Score | Max | % | Status | Rationale |",
    "|--------|-------|-----|---|--------|-----------|",
  );
  ...
  // Findings
  lines.push("", "---", "", `## Findings`, "",
    `**Total: ${findings.length} | Open: ${openFindings.length} | Critical: ${sevCounts.Critical} | High: ${sevCounts.High} | Medium: ${sevCounts.Medium} | Low: ${sevCounts.Low}**`,
    "",
  );
  if (findings.length === 0) {
    ...
    for (const f of findings) {
      ...
      lines.push(`**Domain:** ${f.domain}  |  **Status:** ${f.status}  |  **Confidence:** ${f.confidence || "—"}`);
      lines.push("");
  ...
  lines.push("", "## Uploaded Inputs", "",
    "| File | Type | Extraction Status |",
    "|------|------|------------------|");
  for (const inp of pack.uploadedInputs || []) {
    const status = inp.extractionStatus === "Failed"
      ? `⚠ Failed${inp.extractionSummary ? `: ${inp.extractionSummary.slice(0,60)}` : ""}`
      : inp.extractionStatus;
    lines.push(`| ${inp.fileName || "—"} | ${inp.documentType || "—"} | ${status} |`);
  }
...
function renderHtmlExportBody(pack, summaryText) {
  ...
  parts.push(
    ...
    `<span style="display:inline-block;padding:3px 12px;background:#EB0000;color:#fff;border-radius:4px;font-size:11px;font-weight:700;letter-spacing:0.5px;">CONFIDENTIAL</span>`,
    `<span style="font-size:12px;color:#94A3B8;">Cloud Architecture Review Board Pack</span>`,
    `</div>`,
    `<h1 style="margin:0 0 4px;font-size:26px;font-weight:700;color:#0F172A;letter-spacing:-0.3px;">${esc(proj.name)}</h1>`,
    `<p style="margin:0;font-size:14px;color:#64748B;">Architecture Review Pack  ·  ${esc(cust.name || "")}  ·  Review ID: ${esc(meta.reviewId || "")}</p>`,
    `</div>`
  );
  ...
  parts.push(
    `<nav class="no-print" style="margin:20px 0;padding:14px 18px;background:#F8FAFC;border:1px solid #E2E8F0;border-radius:8px;">`,
    `<p style="margin:0 0 8px;font-size:11px;font-weight:600;color:#94A3B8;letter-spacing:0.5px;text-transform:uppercase;">Contents</p>`,
    `<div style="display:flex;flex-wrap:wrap;gap:6px 16px;">`,
    ...navSections.map(([href, label]) =>
      `<a href="${href}" style="font-size:13px;color:#0059C8;text-decoration:none;white-space:nowrap;">${esc(label)}</a>`),
    `</div></nav>`
  );

  /* ── METADATA CARD ── */
  parts.push(
    `<div id="section-metadata" style="margin:20px 0 32px;padding:20px 24px;background:#F8FAFC;border:1px solid #E2E8F0;border-radius:8px;">`,
  ...
  /* ── REVIEWER DECISION ── */
  {
    ...
    const decIcon = dec === "Approved" || dec === "Conditionally Approved" ? "✓"
      : dec === "Needs Revision" ? "↻"
        : dec === "Needs Remediation" ? "✗" : "·";
    const displayDec = dec || "Pending Review";
    ...
    parts.push(
      ...
      hasDecision ? `<p style="margin:0 0 4px;font-size:13px;color:${decStyle.fg};opacity:0.85;">${esc(dc.reviewerName || "Not recorded")} · ${esc(dc.recordedAt || "Not recorded")}</p>` : "",
    ...
    // Approval Conditions — Critical/High open blockers
    const blockers = findings.filter((f) => ["Critical", "High"].includes(f.severity) && f.status !== "Closed");
    if (blockers.length > 0) {
      parts.push(
        `<div style="padding:14px 18px;background:#FFF5F5;border:1px solid #FCA5A5;border-top:none;border-radius:0 0 8px 8px;">`,
        `<p style="margin:0 0 8px;font-size:12px;font-weight:700;color:#D92B2B;text-transform:uppercase;letter-spacing:0.3px;">Approval Conditions — ${blockers.length} blocking item${blockers.length !== 1 ? "s" : ""} must be resolved</p>`,
        `<ul style="margin:0;padding-left:18px;">`,
        ...blockers.map((f) => `<li style="margin-bottom:4px;font-size:13px;color:#374151;"><strong style="color:${f.severity === "Critical" ? "#D92B2B" : "#C85000"};">[${esc(f.severity)}]</strong> ${esc(f.title || "Untitled finding")}</li>`),
        `</ul></div>`,
      );
    } else if (!dec || dec === "Approved") {
      parts.push(
        `<div style="padding:10px 18px;background:#F0FDF4;border:1px solid #86EFAC;border-top:none;border-radius:0 0 8px 8px;font-size:13px;color:#065F46;">✓ No Critical or High severity blockers. Decision can proceed.</div>`,
      );
    }
    parts.push(`</div>`);
  }
  ...
  if (overallScore !== null) {
    parts.push(
      `<div id="section-score" style="margin-bottom:32px;">`,
  ...
  /* ── SCOPE & METHODOLOGY ── */
  {
    ...
    parts.push(
      divider,
      `<div id="section-methodology" style="margin-bottom:32px;">`,
      `<h2 style="margin:0 0 12px;font-size:18px;font-weight:600;color:#0F172A;">Assessment Scope &amp; Methodology</h2>`,
  ...
  {
    const risks = pack.riskRegister || [];
    const RISK_POS = { Critical: ["High","High"], High: ["High","Medium"], Medium: ["Medium","Medium"], Low: ["Low","Low"] };
    const lMap = { High: 2, Medium: 1, Low: 0 };
    parts.push(divider);
    parts.push(
      `<div id="section-risks" style="margin-bottom:32px;">`,
      `<h2 style="margin:0 0 16px;font-size:18px;font-weight:600;color:#0F172A;">Risk Register <span style="font-size:13px;font-weight:400;color:#64748B;">(${risks.length})</span></h2>`,
    );
    if (risks.length === 0) {
      parts.push(`<p style="color:#64748B;font-style:italic;">No risks recorded. Risk items are derived from open Critical and High severity findings.</p>`);
    } else {
      parts.push(
        ...
        `<th style="padding:8px 12px;text-align:left;">Residual Risk</th>`,
        ...
        ...risks.map((r, i) => {
          ...
          return `<tr style="background:${bg};border-bottom:1px solid #E2E8F0;">` +
            ...
            `<td style="padding:8px 12px;">${esc(r.impact || "Medium")}</td>` +
            `<td style="padding:8px 12px;"><span style="padding:2px 8px;border-radius:10px;font-size:11px;font-weight:600;background:${sevStyles[residual] || "#F1F5F9"};color:#374151;">${esc(residual)}</span></td>` +
            `<td style="padding:8px 12px;">${esc(r.riskOwner || r.owner || "TBC")}</td>` +
            `<td style="padding:8px 12px;">${esc(r.status || "Open")}</td>` +
            `</tr>`;
        }),
        `</tbody></table></div>`,
      );
  ...
  parts.push(divider);
  parts.push(
    `<div id="section-findings" style="margin-bottom:32px;">`,
  ...
  parts.push(divider);
  parts.push(
    `<div id="section-evidence" style="margin-bottom:32px;">`,
  ...
  parts.push(divider);
  parts.push(
    `<div id="section-inputs" style="margin-bottom:32px;">`,
  ...
  parts.push(divider);
  parts.push(
    `<div id="section-requirements" style="margin-bottom:32px;">`,
